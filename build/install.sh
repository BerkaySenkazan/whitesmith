...
Now you should access CLI script: $ whitesmith --help
Alternatively you can access CLI script via poetry run: $ poetry run whitesmith --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_WHITESMITH_COMPLETE=source_bash whitesmith)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_WHITESMITH_COMPLETE=source_zsh whitesmith)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_WHITESMITH_COMPLETE=source_fish whitesmith)' >> ~/.config/fish/completions/whitesmith.fish
EOF