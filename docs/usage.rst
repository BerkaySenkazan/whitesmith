...
.. code-block:: bash
    ...
    $ whitesmith --help
    Usage: whitesmith [OPTIONS] COMMAND [ARGS]...
    ...
    Commands:
      serve  whitesmith CLI serve command.
    $ whitesmith serve --help
    Usage: whitesmith serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name whitesmith -p 8000:8000 whitesmith:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/whitesmith:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  whitesmith
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``whitesmith.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean whitesmith image
      install          Install whitesmith with poetry
      metrics          Run whitesmith metrics checks
      unit-test        Run whitesmith unit tests
      integration-test  Run whitesmith integration tests
      docs             Build whitesmith documentation
...
* RedisClient ``whitesmith.app.utils.redis``
* AiohttpClient ``whitesmith.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from whitesmith.app.utils import RedisClient
...
.. literalinclude:: ../whitesmith/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``whitesmith.config`` submodule.
...
.. literalinclude:: ../whitesmith/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``whitesmith.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../whitesmith/app/router.py
    :language: python
